//image loader.
// https://github.com/akademy/ImageLoader
{
    function ImageLoader( settings )
    {
        // Constructor
        {
            ...
            if( settings.onAllLoaded != undefined )
                thatImageLoader.onAllLoaded = settings.onAllLoaded;

            this.onImageLoaded = null;
            if( settings.onImageLoaded != undefined )
                thatImageLoader.onImageLoaded = settings.onImageLoaded;
            ...
            for( i = 0; i < this.imageCount; i++ )
            {
                ...
                thatImageLoader.images[i] = new LoadImage( name, id, i, settings.images[i].file, this );
            }
                ...
                for( j = 0; j < nameArray.length; j++ )
                    for( i = 0; i < thatImageLoader.imageCount; i++ )
                        if( nameArray[j] == this.images[i].name )
                            if( thatImageLoader.images[i].loaded == false )
                                return false;
            ...
            for( i = 0; i < this.imageCount; i++ )
                if( position == this.images[i].position )
                {
                    ...
                    if( thatImageLoader.onImageLoaded != null )
                        thatImageLoader.onImageLoaded( this.images[i].name, this.images[i].image );
                }
